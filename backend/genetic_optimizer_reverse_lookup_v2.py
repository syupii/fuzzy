...
class PriorityConfig:
    ...
    """
    # 各項目の優先度（1-10スケール）
    research_intensity_priority: float = 7.0
    advisor_style_priority: float = 5.0
    team_work_priority: float = 5.0
    workload_priority: float = 6.0
    theory_practice_priority: float = 5.0
    skill_development_priority: float = 6.0
    lab_atmosphere_priority: float = 5.0
    flexibility_priority: float = 5.0
    publication_opportunity_priority: float = 7.0
    interdisciplinary_priority: float = 4.0
    communication_style_priority: float = 4.0
    
    # 分野重視度（1-10スケール）
    field_priority: float = 5.0
    
    def to_normalized_array(self) -> np.ndarray:
        """優先度を正規化した配列として返す [0, 1]"""
        priorities = [
            self.research_intensity_priority,
            self.advisor_style_priority,
            self.team_work_priority,
            self.workload_priority,
            self.theory_practice_priority,
            self.skill_development_priority,
            self.lab_atmosphere_priority,
            self.flexibility_priority,
            self.publication_opportunity_priority,
            self.interdisciplinary_priority,
            self.communication_style_priority
        ]
        return np.array(priorities) / 10.0
    
    def get_normalized_field_priority(self) -> float:
        """分野重視度を正規化して返す [0, 1]"""
        return self.field_priority / 10.0
    
    def to_dict(self) -> Dict[str, float]:
        """辞書形式で返す"""
        return {
            "research_intensity_priority": self.research_intensity_priority,
            "advisor_style_priority": self.advisor_style_priority,
            "team_work_priority": self.team_work_priority,
            "workload_priority": self.workload_priority,
            "theory_practice_priority": self.theory_practice_priority,
            "skill_development_priority": self.skill_development_priority,
            "lab_atmosphere_priority": self.lab_atmosphere_priority,
            "flexibility_priority": self.flexibility_priority,
            "publication_opportunity_priority": self.publication_opportunity_priority,
            "interdisciplinary_priority": self.interdisciplinary_priority,
            "communication_style_priority": self.communication_style_priority,
...
class StudentProfile:
    ...
    【最適化対象】
    - criteria_values: 11項目の評価値 [0, 1]
    - field_interests: 興味分野と興味度 {field_id: [0, 1]}
    
    【固定値（最適化対象外）】
    - criteria_priorities: 11項目の優先度（PriorityConfigから設定）
    - field_priority: 分野重視度（PriorityConfigから設定）
    """
    ...
    def __init__(
        ...
        """
        Args:
            criteria_values: 11項目の評価値 [0, 1]の範囲（最適化対象）
            field_interests: 分野興味度 {field_id: [0, 1]}（最適化対象）
            priority_config: 優先度設定（固定値）
        """
        # 最適化対象: 評価値
        ...
        self.criteria_priorities = self.priority_config.to_normalized_array()
        self.field_priority = self.priority_config.get_normalized_field_priority()
    ...
    def copy(self):
        ...
        new_profile = StudentProfile(
            ...
            self.priority_config  # 優先度設定は共有（固定値なので）
        )
...
class GeneticOptimizer:
    ...
    【最適化対象】
    - 評価値（criteria_values）
    - 興味分野（field_interests）
    
    【固定値（最適化対象外）】
    - 優先度（criteria_priorities）
    - 分野重視度（field_priority）
    """
    
    def __init__(self, config: GAConfig = None, priority_config: PriorityConfig = None):
    ...
    def initialize_population(self) -> List[StudentProfile]:
        """初期個体群の生成"""
        return [
            StudentProfile(priority_config=self.priority_config) 
    ...
    def crossover(
        ...
        # 優先度設定は固定（親から共有）
        child1 = StudentProfile(child1_values, child1_fields, self.priority_config)
        child2 = StudentProfile(child2_values, child2_fields, self.priority_config)
...
def analyze_single_lab(
    ...
    logger.info(f"分野重視度（固定）: {priority_config.field_priority}")
    
    # 遺伝的アルゴリズム実行
    optimizer = GeneticOptimizer(config, priority_config)
    ...
    result = {
        ...
        "priority_config": priority_config.to_dict(),
    ...
    logger.info(f"  基本項目（最適化結果）:")
    for i, criterion in enumerate(BASIC_CRITERIA):
        value = best_student.criteria_values[i] * 9 + 1
        priority = best_student.criteria_priorities[i] * 9 + 1
        logger.info(f"    {criterion}: 値={value:.2f} (優先度={priority:.1f}固定)")
    logger.info(f"  分野重視度: {best_student.field_priority * 9 + 1:.2f}（固定）")
    logger.info(f"  分野興味（最適化結果）:")
...
def analyze_all_labs(
    config: GAConfig, 
    priority_config: PriorityConfig,
    output_dir: str = "results/genetic_optimization"
):
    """全研究室の最適学生プロファイルを探索"""
    labs = load_lab_database()
    ...
    for i, lab in enumerate(labs, 1):
        ...
        result = analyze_single_lab(lab_id, config, priority_config)
        if result:
    ...
    summary = {
        ...
        "note": "優先度と分野重視度は固定値。最適化対象は評価値と興味分野のみ。",
        "results": all_results
    }
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results2/genetic_optimization", help="出力ディレクトリ")
    ...
    # 優先度設定（固定値）
    parser.add_argument("--field_priority", type=float, default=5.0, help="分野重視度（1-10、固定値）")
    parser.add_argument("--ri_priority", type=float, default=7.0, help="research_intensity優先度（固定値）")
    parser.add_argument("--pub_priority", type=float, default=7.0, help="publication_opportunity優先度（固定値）")
    ...
    # 優先度設定
    priority_config = PriorityConfig(
        research_intensity_priority=args.ri_priority,
        publication_opportunity_priority=args.pub_priority,
        field_priority=args.field_priority
    )
    
    if args.all:
        analyze_all_labs(config, priority_config, args.output)
    elif args.lab_id:
        result = analyze_single_lab(args.lab_id, config, priority_config)
        if result:
