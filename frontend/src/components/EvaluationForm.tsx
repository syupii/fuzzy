...
const EvaluationForm: React.FC<EvaluationFormProps> = ({ onResults }) => {
  ...
  const handleEvaluate = async () => {
    ...
    try {
      // 基本的な評価を行う（拡張版がない場合の後方互換性）
      const response = await apiService.evaluateBasicCompatibility ? 
        await apiService.evaluateBasicCompatibility(preferences) :
        await apiService.evaluateCompatibility(preferences);
  ...
  const loadDemoData = async () => {
    try {
      ...
      // 基本的な設定のみ抽出
      const basicPrefs: EvaluationPreferences = {
        research_intensity: response.demo_preferences.research_intensity,
        advisor_style: response.demo_preferences.advisor_style,
        team_work: response.demo_preferences.team_work,
        workload: response.demo_preferences.workload,
        theory_practice: response.demo_preferences.theory_practice,
      };
